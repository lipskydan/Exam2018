...
public:
    ...
    void set_populerity(Date& c_date) {
        ...
        if (this->date.date_diff(c_date) < 0)
        {
            popularity = -100;
            return;
        }
...
private:
    ...
    void show_all_songs(Date& c_date) {
        ...
        for(auto i = st.begin(); i != st.end(); i++, index++) {
            cout << index << ". " << i->get_song_name() << " - " << i->get_group_name() << " - " << i->get_genre_name() << " - "
            << "   score: " << i->get_popularity() << endl;
        }
    }
    void show_by_group(Date& c_date, string group_name) {
        ...
        for(auto i : vec) {
            i.set_populerity(c_date);
            if (i.get_group_name() == group_name && i.get_popularity() != -100) {
                st.insert(i);
            }
        }
        int index = 1;
...
private:
    void load_songs() {
        ...
        while (file >> tmp_str) {
            index = tmp_str;
            file >> song_name >> song_genre >> group_name >> group_preferences >> year >> month >> day;
        ...
        string s;
        ifstream inp("/Users/danial/Desktop/exam/songs_list.txt");
        getline(inp, s, '\0');
